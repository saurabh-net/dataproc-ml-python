...
class TestNormalizeModelParams(unittest.TestCase):
    ...
    def test_unsupported_types_are_rejected(self):
        for value in [42, ["temperature"], object()]:
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    _model_params.normalize_model_params(value)
...
class TestJsonStringForm(unittest.TestCase):
    """The spec defines model_params as a JSON string, and a SQL argument can
    only ever arrive as one."""
    ...
    def test_a_json_string_and_a_dictionary_agree(self):
        params = {"generation_config": {"temperature": 0.2}}
        self.assertEqual(
            _model_params.normalize_model_params(json.dumps(params)),
            _model_params.normalize_model_params(params),
        )

    def test_numbers_decode_to_plain_floats(self):
        # Not Decimal, which is what a Spark named_struct or a VARIANT would
        # produce, and which is not JSON serializable further down.
        flat = _model_params.normalize_model_params(
            '{"generation_config": {"temperature": 0.7}}'
        )
        self.assertIsInstance(flat["temperature"], float)

    def test_an_empty_string_means_no_parameters(self):
        for value in ["", "   "]:
            with self.subTest(value=value):
                self.assertEqual(
                    _model_params.normalize_model_params(value), {}
    ...
    def test_malformed_json_is_rejected(self):
        for value in ["{not json", "temperature=0", "{'a': 1}", "{"]:
            with self.subTest(value=value):
                with self.assertRaises(ValueError) as ctx:
                    _model_params.normalize_model_params(value)
    ...
    def test_json_that_is_not_an_object_is_rejected(self):
        for value in ["[1, 2]", '"hello"', "42", "null"]:
            with self.subTest(value=value):
                with self.assertRaises(ValueError) as ctx:
                    _model_params.normalize_model_params(value)
                self.assertIn("must be a JSON object", str(ctx.exception))

    def test_contents_is_rejected_in_the_string_form_too(self):
        with self.assertRaises(ValueError) as ctx:
            _model_params.normalize_model_params('{"contents": ["hello"]}')
        self.assertIn("should not contain contents", str(ctx.exception))
