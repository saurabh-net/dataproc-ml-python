...
field of the result, using the same wording as the equivalent BigQuery AI
function, so that a single bad row never fails an entire Spark stage.
"""
...
# Reported when a structured response cannot be parsed as JSON.
PARSE_FAILURE_STATUS = (
    "Failed to parse the response - please check the full_response field."
)
...
def to_status_message(exception: BaseException) -> str:
    ...
    The wording matches BigQuery's AI functions: request errors are surfaced
    verbatim, quota messages are passed through, and everything else is
    summarized as a retryable error.
