...
def _as_dict(
    model_params: Optional[Union[str, Dict[str, Any]]]
) -> Dict[str, Any]:
    """Decodes ``model_params`` into a dictionary.

    The string form is what the spec defines and the only form that can reach
    an executor as a column value. A dictionary is also accepted because
    building one is more natural from Python.
    """
    if model_params is None:
        return {}

    if isinstance(model_params, str):
        text = model_params.strip()
        if not text:
            return {}
        try:
            ...
            raise ValueError(
                "The model_params is not valid JSON. It must be a JSON "
                "object matching the generateContent request body, for "
                'example \'{"generation_config": {"temperature": 0.2}}\'. '
                f"Cause: {e}"
            ) from e
        if not isinstance(decoded, dict):
            raise ValueError(
                "The model_params must be a JSON object, got a "
    ...
    raise ValueError(
        "model_params must be a JSON string or a dictionary matching the "
        f"generateContent request body, got {type(model_params).__name__}."
    )
...
def normalize_model_params(
    ...
    Args:
        model_params: The user supplied request body, as a JSON string or as
            an already decoded dictionary, or None.
    ...
    Raises:
        ValueError: If the parameters are not valid JSON, contain
            ``contents``, declare a response schema that conflicts with
            ``output_schema``, or do not describe a valid request body.
    """
